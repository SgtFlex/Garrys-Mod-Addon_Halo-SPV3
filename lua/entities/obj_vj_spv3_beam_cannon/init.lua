...
ENT.RadiusDamage = 65 -- How much damage should it deal? Remember this is a radius damage, therefore it will do less damage the farther away the entity is from its enemy
...
function ENT:CustomOnInitialize()
	...
	self:SetNoDraw(true)
	ParticleEffectAttach("hcea_hunter_frg_proj", PATTACH_ABSORIGIN_FOLLOW, self, 0)
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:Think()
...
if (SERVER) then 
local r = math.random()
local chance_for_lightning = 0.1
local shouldLightning = r >= chance_for_lightning -- if shouldLightning... 

ParticleEffectAttach("hcea_hunter_frg_proj_lightning_trcr_s",PATTACH_ABSORIGIN_FOLLOW, self, 0)
