...
ENT.RadiusDamage = 30 * GetConVarNumber("vj_spv3_DamageModifier")
...
function ENT:CustomOnInitialize()
	-- ParticleEffect("doom_rev_muzzle",self:GetPos(),Angle(-90,0,0),nil)
	self.RadiusDamage = 20 * GetConVarNumber("vj_sent_sentinelDamageModifier")-- How much damage should it deal? Remember this is a radius damage, therefore it will do less damage the farther away the entity is from its enemy
	...
	-- ParticleEffectAttach("doom_caco_nade", PATTACH_ABSORIGIN_FOLLOW, self, 0)
	-- ParticleEffectAttach("doom_rev_missile_trail", PATTACH_ABSORIGIN_FOLLOW, self, 0)
	light = ents.Create("env_sprite")
	light:SetKeyValue("model","orangecore2.vmt")
	light:SetKeyValue("rendercolor","250 250 0")
	light:SetKeyValue("scale","0.6")
	light:SetPos(self:GetPos())
	light:SetParent(self)
	light:Spawn()
	light:Activate()
	util.SpriteTrail(self, 0,Color(200,200,200,200),true,20,5,.2,20,"trails/smoke")
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:DeathEffects(data,phys)
	...
	-- ParticleEffect("doom_rev_missile_blast",self:GetPos(),Angle(0,0,0),nil)
