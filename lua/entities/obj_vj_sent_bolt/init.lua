...
ENT.RadiusDamage = 5 * GetConVarNumber("vj_sent_sentinelDamageModifier") -- How much damage should it deal? Remember this is a radius damage, therefore it will do less damage the farther away the entity is from its enemy
...
ENT.DirectDamage = 3 * GetConVarNumber("vj_sent_sentinelDamageModifier")-- How much damage should it do when it hits something
...
function ENT:CustomOnInitialize()
	if (IsValid(self:GetOwner()) && (self:GetOwner():GetClass()=="npc_vj_sent_h2enforcershielded")) then
	...
	light:SetKeyValue("rendercolor","250 0 0")
	light:SetKeyValue("scale","0.25")
	light:SetPos(self:GetPos())
	light:SetParent(self)
	light:Spawn()
	light:Activate()
	self.trail = util.SpriteTrail(self, 0,Color(120,0,255,255),true,20,5,.3,20,"trails/smoke")
	timer.Create("Trail"..self:GetCreationID(), 0.1, 1, function() if (IsValid(self)) then self.trail:Remove() end end)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:DeathEffects(data,phys) 
	-- ParticleEffect("hcea_hunter_needler_pistol_impact",self:GetPos(),Angle(0,0,0),nil)
end
---------------------------------------------------------------------------------------------------------------------------------------------
